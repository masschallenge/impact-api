...
class TestCriterionDetailView(APITestCase):
    def test_get(self):
        ...
        with self.login(email=self.basic_user().email):
            url = reverse(CriterionOptionSpecDetailView.view_name, args=[option_spec.id])
    ...
    def test_patch_no_such_object(self):
        ...
        data = {'option': 'Patched Option',
                ...
                'count': 1}
        with self.login(email=self.basic_user().email):
            url = reverse(CriterionOptionSpecDetailView.view_name, args=[option_spec_id])
    ...
    def test_patch_bad_key_in_data(self):
        ...
        with self.login(email=self.basic_user().email):
            url = reverse(CriterionOptionSpecDetailView.view_name, args=[option_spec.id])
            response = self.client.patch(url, data)
            note = valid_keys_note(CriterionOptionSpecDetailView.helper_class.INPUT_KEYS)
